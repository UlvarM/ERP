from PySide6.QtWidgets import (
    ...
    QTableWidget, QTableWidgetItem, QComboBox, QMessageBox, QSpinBox
)
from PySide6.QtCore import Qt
from database import SessionLocal
from logic import get_projects, get_project_parts, start_project_deduct_inventory, add_history_entry
...
class StartProjectWidget(QWidget):
    ...
    def init_ui(self):
        ...
        details_label.setStyleSheet("font-size: 16px; font-weight: bold; margin-top: 20px;")
        layout.addWidget(details_label)
        
        self.parts_table = QTableWidget()
        self.parts_table.setColumnCount(5)
        self.parts_table.setHorizontalHeaderLabels(["Materjali ID", "Materjali Nimi", "Vajalik Kogus", "Laos Kogus", "Staatus"])
        ...
        with SessionLocal() as db:
            ...
            for i, part in enumerate(project_parts):
                ...
                if material.stock_qty >= required_qty:
                    ...
                    status_item = QTableWidgetItem(f"Puudus ({required_qty - material.stock_qty} puudu)")
            ...
            if not all_available:
                QMessageBox.warning(self, "Hoiatus", "Mõnel materjalil on puudus. Vaata staatusetabelit.")
            
    def start_project(self):
        """Käivita projekt ja vähenda laoseisu vastavalt kordajale."""
        if self.current_project_id is None:
            QMessageBox.warning(self, "Hoiatus", "Palun vali ja lae projekt enne käivitamist.")
            return
        ...
        if reply == QMessageBox.Yes:
            with SessionLocal() as db:
                try:
                    ...
                    add_history_entry(db, "Projekt Käivitatud", f"Käivitas projekti: {project_name} x{multiplier}")
                    
                    QMessageBox.information(self, "Edukas", f"Projekt '{project_name}' käivitatud {multiplier} kord(a). Laos olev kogus uuendatud.")
                    self.load_project_details()
                except ValueError as e:
                    QMessageBox.critical(self, "Viga", str(e))
                except Exception as e:
                    QMessageBox.critical(self, "Viga", f"Projekti käivitamine ebaõnnestus: {str(e)}")
